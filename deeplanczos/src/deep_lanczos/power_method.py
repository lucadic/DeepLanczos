...
def PowerMethodStep(loss, par, batch_iterator, V, batch_parser=standard_parser, show_pbar=False):
    ...
    for batch in iterator: 
        ...
        count+=batch_parser(batch)[0].shape[0]
    HVP = jtu.tree_map(lambda x: x/count, HVP) 
    alpha = dot_product(HVP, HVP)/ dot_product(V, V)  
    V_new = normalize(HVP)
    return V_new, alpha  

def PowerMethodIterate(loss, par, batch_iterable, num_iterations, V0=None, batch_parser=standard_parser, show_pbar=False): 
    ...
    V = V0.copy() 
    History = {'alpha' : [], 'iteration': []}
    for iteration in range(num_iterations): 
       batch_iterator = iter(batch_iterable)  # Reset iterator for each iteration
       V_new, alpha = PowerMethodStep(loss, par, batch_iterator, V, batch_parser=batch_parser, show_pbar=False) 
       V = V_new 
